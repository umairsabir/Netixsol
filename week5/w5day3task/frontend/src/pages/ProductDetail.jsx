...
import { Minus, Plus, ShoppingBag, Globe, Leaf, Clock, Coffee, Droplets, Heart, Star, MessageSquare, Send, Trash2 } from 'lucide-react';
...
const ProductDetail = () => {
    ...
    useEffect(() => {
        const fetchProductAndReviews = async () => {
            try {
                ...
                // Fetch reviews from NestJS (Port 3000)
                const revRes = await axios.get(`http://localhost:3000/api/reviews/product/${id}`);
    ...
    // Setup Socket.IO dynamic listeners for live reviews
    useEffect(() => {
        const handleBroadcastReview = (e) => {
            const { review } = e.detail;
            
            // Ignore socket broadcast for the author's own review to avoid duplication
            const reviewAuthorId = review.user?._id || review.user;
            if (user && reviewAuthorId && reviewAuthorId.toString() === user._id.toString()) {
                return;
            }
        ...
        document.addEventListener('broadcast-review', handleBroadcastReview);
        return () => {
            document.removeEventListener('broadcast-review', handleBroadcastReview);
        };
    }, [id]);
    ...
    const handleAddReview = async (e) => {
        ...
        if (!newComment.trim()) {
            setNotification('Review comment cannot be empty');
            setTimeout(() => setNotification(null), 3000);
            return;
        }
        ...
        try {
            ...
            setNotification(err.response?.data?.message || 'Failed to submit review');
            setTimeout(() => setNotification(null), 3000);
        } finally {
    ...
    const handleAddReply = async (reviewId) => {
        ...
        try {
            const res = await axios.post(
                `http://localhost:3000/api/reviews/${reviewId}/reply`,
                { comment },
                { headers: { Authorization: `Bearer ${token}` } }
            );
            ...
            setNotification('Reply posted successfully!');
            setTimeout(() => setNotification(null), 3000);
        } catch (err) {
            console.error('Error submitting reply:', err.message);
            setNotification('Failed to submit reply');
            setTimeout(() => setNotification(null), 3000);
        }
    };
    ...
    const handleToggleLike = async (reviewId) => {
        const token = sessionStorage.getItem('token');
        if (!token) {
            setNotification('Please login to like reviews');
            setTimeout(() => setNotification(null), 3000);
            return;
        }
    ...
    const handleDeleteReview = async (reviewId) => {
        ...
        try {
            ...
            setNotification('Failed to delete review');
            setTimeout(() => setNotification(null), 3000);
        }
    };
    ...
    return (
        <div className="bg-white dark:bg-[#0A0A0A] min-h-screen font-['Montserrat'] pb-20 transition-colors duration-300">
            ...
            {/* Reviews Section */}
            <div className="bg-white dark:bg-[#0A0A0A] border-t border-gray-100 dark:border-gray-900 py-20 transition-colors duration-300">
                <div className="container mx-auto px-6 max-w-7xl">
                    <h2 className="text-[28px] md:text-[32px] font-bold text-black dark:text-white font-['Prosto_One'] uppercase mb-12">
                        Customer Reviews
                    </h2>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
                        ...
                        <div className="lg:col-span-1 space-y-10">
                            {/* Stars Summary */}
                            <div className="bg-[#F8F8F8] dark:bg-[#111] p-8 rounded-2xl border border-gray-100 dark:border-gray-800">
                                <h3 className="text-[12px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-4">Rating Overview</h3>
                                <div className="flex items-baseline gap-3 mb-4">
                                    <span className="text-[48px] font-bold text-black dark:text-white font-['Prosto_One']">
                                        {reviews.length > 0 ? (reviews.reduce((acc, r) => acc + r.rating, 0) / reviews.length).toFixed(1) : '0.0'}
                                    </span>
                                    <span className="text-[16px] text-gray-400 font-bold">/ 5.0</span>
                                </div>
                                
                                <div className="flex items-center gap-1 mb-6">
                                    {[1, 2, 3, 4, 5].map((s) => {
                                        const avg = reviews.length > 0 ? (reviews.reduce((acc, r) => acc + r.rating, 0) / reviews.length) : 0;
                                        return <Star key={s} size={20} fill={s <= Math.round(avg) ? "#FBBF24" : "none"} stroke={s <= Math.round(avg) ? "#FBBF24" : "#D1D5DB"} strokeWidth={1.5} />;
                                    })}
                                    <span className="text-[12px] text-gray-500 dark:text-gray-400 font-bold ml-2">({reviews.length} reviews)</span>
                                </div>
                                ...
                                <div className="space-y-2">
                                    {[5, 4, 3, 2, 1].map((ratingVal) => {
                                        ...
                                        return (
                                            <div key={ratingVal} className="flex items-center gap-3 text-[12px] font-bold">
                                                <span className="w-3 text-black dark:text-white">{ratingVal}</span>
                                                <Star size={12} fill="#FBBF24" stroke="#FBBF24" className="shrink-0" />
                                                <div className="flex-grow h-2 bg-gray-200 dark:bg-gray-800 rounded-full overflow-hidden">
                                                    <div className="h-full bg-black dark:bg-white rounded-full transition-all duration-500" style={{ width: `${pct}%` }}></div>
                                                </div>
                                                <span className="w-8 text-right text-gray-400 dark:text-gray-500 font-bold">{count}</span>
                                            </div>
                                        );
                                    })}
                                </div>
                            ...
                            {/* Add Review Form */}
                            <div className="bg-white dark:bg-[#0A0A0A] border border-gray-100 dark:border-gray-800 p-8 rounded-2xl">
                                <h3 className="text-[16px] font-bold text-black dark:text-white uppercase tracking-wider mb-6">Write a review</h3>
                                {user ? (
                                    <form onSubmit={handleAddReview} className="space-y-6">
                                        {/* Star Selector */}
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Select Rating</label>
                                        ...
                                        {/* Comment Textarea */}
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Your review</label>
                                            <textarea
                                                value={newComment}
                                                onChange={(e) => setNewComment(e.target.value)}
                                                placeholder="Tell us what you think about this tea..."
                                                rows={4}
                                                className="w-full bg-[#F8F8F8] dark:bg-[#111] border border-gray-100 dark:border-gray-800 rounded-xl p-4 text-sm outline-none text-black dark:text-white font-medium placeholder:text-gray-400 focus:border-black dark:focus:border-white transition-colors resize-none animate-none"
                                            />
                                        </div>
                                        ...
                                        <button
                                            ...
                                            className={`w-full h-14 bg-black dark:bg-[#1A1A1A] text-white font-bold tracking-[0.2em] flex items-center justify-center gap-2 hover:bg-[#222] dark:hover:bg-[#2A2A2A] transition-all uppercase text-[12px] rounded-xl shadow-md cursor-pointer ${submitLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
                                        >
                                            <Send size={16} />
                                    ...
                                    <div className="text-center py-6">
                                        <p className="text-[12px] text-gray-500 dark:text-gray-400 font-bold mb-4">Please log in to leave your feedback.</p>
                                        <button
                                            onClick={() => navigate('/login')}
                                            className="w-full h-14 border border-black dark:border-white text-black dark:text-white hover:bg-black dark:hover:bg-white hover:text-white dark:hover:text-black font-bold tracking-[0.2em] transition-all uppercase text-[12px] rounded-xl cursor-pointer"
                                        >
                                            Login / Register
                                        </button>
                                    </div>
                        ...
                        <div className="lg:col-span-2 space-y-8">
                            <h3 className="text-[12px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest border-b border-gray-100 dark:border-gray-800 pb-4">
                                Reviews ({reviews.length})
                            </h3>

                            {reviews.length === 0 ? (
                                <div className="text-center py-20 bg-[#F8F8F8] dark:bg-[#111] rounded-2xl border border-gray-100 dark:border-gray-800">
                                    <p className="text-[13px] text-gray-400 font-bold uppercase tracking-wider">No reviews yet</p>
                                    <p className="text-[12px] text-gray-500 font-medium mt-1">Be the first to share your thoughts!</p>
                                </div>
                            ) : (
                                <div className="space-y-8 divide-y divide-gray-100 dark:divide-gray-800">
                                    {reviews.map((rev) => {
                                        const reviewUser = rev.user || {};
                                        const hasLiked = user && Array.isArray(rev.likes) && rev.likes.some((id) => id.toString() === user._id);
                                        const isAdmin = user && ['admin', 'superadmin'].includes(user.role);

                                        return (
                                            <div key={rev._id} className="pt-8 first:pt-0 space-y-4">
                                                {/* Header */}
                                                <div className="flex items-start justify-between">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-[#94a3b8] to-[#64748b] text-white flex items-center justify-center font-bold uppercase shadow-sm">
                                                            {reviewUser.name ? reviewUser.name.charAt(0) : '?'}
                                                        </div>
                                                        <div>
                                                            <div className="flex items-center gap-2">
                                                                <h4 className="text-[13px] font-bold text-black dark:text-white uppercase tracking-wider">
                                                                    {reviewUser.name || 'Anonymous User'}
                                                                </h4>
                                                                {reviewUser.role && reviewUser.role !== 'user' && (
                                                                    <span className="bg-purple-100 dark:bg-purple-950/30 text-purple-600 dark:text-purple-400 text-[8px] font-bold px-1.5 py-0.5 rounded uppercase tracking-widest">
                                                                        {reviewUser.role}
                                                                    </span>
                                                                )}
                                                            </div>
                                                    ...
                                                    <div className="flex items-center gap-0.5">
                                                        {[1, 2, 3, 4, 5].map((star) => (
                                                            <Star
                                                                ...
                                                                stroke={star <= rev.rating ? "#FBBF24" : "#D1D5DB"}
                                                                strokeWidth={1.5}
                                                            />
                                                        ))}
                                                    </div>
                                                ...
                                                {/* Review Comment */}
                                                <p className="text-[13px] text-gray-600 dark:text-gray-300 font-medium leading-relaxed">
                                                    {rev.comment}
                                                </p>
                                                ...
                                                <div className="flex items-center gap-6 text-[11px] font-bold text-gray-400 uppercase tracking-widest">
                                                    ...
                                                    <button
                                                        ...
                                                        className={`flex items-center gap-1.5 hover:text-black dark:hover:text-white transition-colors cursor-pointer ${hasLiked ? 'text-pink-500' : ''}`}
                                                    >
                                                        <Heart size={14} fill={hasLiked ? "currentColor" : "none"} strokeWidth={2} />
                                                        <span>{Array.isArray(rev.likes) ? rev.likes.length : 0} Likes</span>
                                                    </button>
                                                    ...
                                                    {(isAdmin || (user && reviewUser._id === user._id)) && (
                                                        <button
                                                            onClick={() => handleDeleteReview(rev._id)}
                                                            className="flex items-center gap-1.5 hover:text-red-500 text-red-400 dark:text-red-900/40 transition-colors cursor-pointer ml-auto"
                                                        >
                                                ...
                                                {rev.replies && rev.replies.length > 0 && (
                                                    <div className="ml-8 pl-4 border-l-2 border-gray-100 dark:border-gray-800 space-y-4 pt-2">
                                                        {rev.replies.map((rep, idx) => (
                                                            <div key={idx} className="space-y-1">
                                                                <div className="flex items-center gap-2">
                                                                    <span className="text-[11px] font-bold text-black dark:text-white uppercase tracking-wider">{rep.name}</span>
                                                                    <span className="text-[8px] text-gray-400 font-bold">
                                                                        {new Date(rep.createdAt).toLocaleDateString()}
                                                                    </span>
                                                                </div>
                                                                <p className="text-[12px] text-gray-500 dark:text-gray-450 font-medium leading-relaxed">{rep.comment}</p>
                                                            </div>
                                                        ))}
                                                    </div>
                                                )}
                                                ...
                                                {activeReplyId === rev._id && (
                                                    <div className="ml-8 pl-4 border-l-2 border-gray-100 dark:border-gray-800 flex items-center gap-3 pt-2">
                                                        <input
                                                            type="text"
                                                            value={replyComment[rev._id] || ''}
                                                            onChange={(e) => setReplyComment({ ...replyComment, [rev._id]: e.target.value })}
                                                            placeholder={`Reply to ${reviewUser.name || 'user'}...`}
                                                            className="flex-grow bg-[#F8F8F8] dark:bg-[#111] border border-gray-100 dark:border-gray-800/60 rounded-xl px-4 py-2.5 text-xs outline-none text-black dark:text-white font-medium focus:border-black dark:focus:border-white transition-colors"
                                                        />
                                                        <button
                                                            onClick={() => handleAddReply(rev._id)}
                                                            className="p-2.5 bg-black dark:bg-[#1A1A1A] text-white hover:bg-gray-800 rounded-xl transition-colors cursor-pointer"
                                                        >
                                                            <Send size={12} />
                                                        </button>
                                                    </div>
