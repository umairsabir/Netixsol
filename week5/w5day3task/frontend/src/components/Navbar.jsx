...
import { Search, User, ShoppingBag, X, Sun, Moon, Menu, Bell } from 'lucide-react';
                ...
                {user && (
                    <div className="relative">
                        <motion.button
                            ...
                            whileHover={{ scale: 1.1 }}
                            whileTap={{ scale: 0.9 }}
                            className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors relative flex items-center justify-center text-gray-700 dark:text-gray-300"
                        >
                            <Bell size={20} />
                            {unreadCount > 0 && (
                                <span className="absolute -top-0.5 -right-0.5 bg-red-500 text-white text-[8px] font-bold w-4.5 h-4.5 flex items-center justify-center rounded-full border-2 border-white dark:border-[#0A0A0A] animate-pulse">
                                    {unreadCount}
                                </span>
                            )}
                        </motion.button>
                            ...
                            {isNotifDropdownOpen && (
                                <>
                                    ...
                                    <motion.div
                                        ...
                                        className="absolute right-0 mt-3 w-80 bg-white dark:bg-[#1A1A1A] rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-800 p-2.5 z-50 max-h-[400px] overflow-y-auto scrollbar-hide"
                                    >
                                        <div className="px-3 py-2 border-b border-gray-50 dark:border-gray-800 mb-2 flex justify-between items-center">
                                            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Notifications</span>
                                            {unreadCount > 0 && (
                                                <span className="text-[9px] bg-red-50 dark:bg-red-950/30 text-red-600 dark:text-red-400 px-2 py-0.5 rounded-full font-bold">
                                                    {unreadCount} New
                                                </span>
                                            )}
                                        </div>
                                        {notifications.length === 0 ? (
                                            <div className="py-8 text-center text-[11px] font-semibold text-gray-400">
                                            ...
                                            <div className="space-y-1">
                                                {notifications.map((notif) => (
                                                    <div
                                                        ...
                                                        className={`p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-all cursor-pointer relative text-left border border-transparent hover:border-gray-100 dark:hover:border-gray-700/50 ${!notif.isRead ? 'bg-blue-50/30 dark:bg-blue-950/10' : ''}`}
                                                    >
                                                        {!notif.isRead && (
                                                            <div className="absolute right-3 top-4 w-2 h-2 rounded-full bg-blue-500 animate-ping" />
                                                        )}
                                                        <h5 className="text-[11px] font-bold text-black dark:text-white uppercase tracking-wider mb-0.5">
                                                            {notif.title}
                                                        </h5>
                                                        <p className="text-[11px] text-gray-500 dark:text-gray-300 leading-normal font-medium pr-4">
                                                            {notif.message}
                                                        </p>
                                                        <span className="text-[9px] text-gray-400 dark:text-gray-500 font-bold mt-1.5 block">
