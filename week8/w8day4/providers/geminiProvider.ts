...
export class GeminiModel implements Model {
  ...
  async getResponse(request: ModelRequest): Promise<ModelResponse> {
        ...
        output: [
          {
            ...
            role: "assistant",
            status: "completed",
            content: [{ type: "output_text", text: "(The agent is thinking or moving to the next step...)" }],
          } as any
        ],
        usage: {} as any
      } as unknown as ModelResponse;
    ...
    console.error("Gemini Response Data (Unexpected Empty):", JSON.stringify(data, null, 2));
    throw new Error(`Unexpected empty response from Gemini (FinishReason: ${candidate?.finishReason || 'UNKNOWN'})`);
  }
