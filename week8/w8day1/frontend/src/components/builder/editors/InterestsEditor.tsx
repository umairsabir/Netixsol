...
import { ResumeData, InterestItem } from "@/lib/resume-schema"
...
interface InterestsEditorProps {
  data: NonNullable<ResumeData["sections"]>["interests"] | undefined
...
export const InterestsEditor: React.FC<InterestsEditorProps> = ({
  ...
  const handleSave = (item: InterestItem) => {
  ...
  const handleDuplicate = (item: InterestItem) => {
    const newItem = {
        ...item,
  ...
  const handleReorder = (newItems: InterestItem[]) => {
    onChange({
      ...data,
      items: newItems
    })
        ...
        >
          {items.map((item) => (
            <SectionItem
              key={item.id}
              value={item}
              title={item.name || "Untitled Interest"}
              subtitle={item.keywords.join(", ") || "No keywords"}
